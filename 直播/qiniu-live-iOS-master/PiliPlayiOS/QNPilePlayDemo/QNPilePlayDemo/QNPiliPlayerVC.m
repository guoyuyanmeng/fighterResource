...
@interface QNPiliPlayerVC ()

@property (nonatomic, strong) NSMutableArray * dataArray;

@end

@implementation QNPiliPlayerVC
...
- (void)headerRereshing
{
    ...
    //    NSString * urlString = [NSString stringWithFormat:@"/live/stream/list?sessionId=%@&accessToken=%@",[UserInfoClass sheardUserInfo].sessionID,];
    ...
    [HTTPRequestPost hTTPRequest_PostpostBody:dic andUrl:@"live/stream/list" andSucceed:^(NSURLSessionDataTask *task, id responseObject) {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self getDateUrl:self.dataArray[indexPath.row]];
//    [self.navigationController pushViewController:playVC animated:YES];
}

- (void)getDateUrl:(NSDictionary *)dict
{
    NSDictionary * dic = @{@"sessionId":[UserInfoClass sheardUserInfo].sessionID,
                           ...
                           @"publishId":dict[@"publishId"]};
    
    [HTTPRequestPost hTTPRequest_PostpostBody:dic andUrl:@"get/play/stream" andSucceed:^(NSURLSessionDataTask *task, id responseObject) {
        QNPiliPlayVC * playVC = [[QNPiliPlayVC alloc] initWithDic:responseObject];
