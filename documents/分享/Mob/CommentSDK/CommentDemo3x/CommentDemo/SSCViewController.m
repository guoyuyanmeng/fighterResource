...
static NSString *const contentId = @"123456";
...
- (void)viewDidLoad
{
    ...
    //显示工具栏
    [self showCommentToolbarWithContentId:contentId title:@"文章标题"];
...
{
    //显示评论列表
    [self presentCommentListViewControllerWithContentId:contentId
                                                  title:@"文章标题"
                                               animated:YES];
}
...
{
    //显示评论列表
    [self pushCommentListViewControllerWithContentId:contentId
                                               title:@"文章标题"
                                            animated:YES];
}
...
{
    //显示评论界面
    [self presentCommentViewControllerWithContentId:contentId
                                              title:@"文章标题"
                                            comment:nil
                                           animated:YES
                                             result:^(SSCResponseState state, id<ISSCComment> comment, NSError *error) {
                                                 
                                                 if (state == SSCResponseStateSuccess)
                                                 {
                                                     NSLog(@"评论成功!");
                                                 }
                                                 else if (state == SSCResponseStateFail)
                                                 {
                                                     NSLog(@"评论失败! %@", error);
                                                 }
...
{
    //显示评论界面
    [self pushCommentViewControllerWithContentId:contentId
                                           title:@"文章标题"
                                         comment:nil
                                        animated:YES
                                          result:^(SSCResponseState state, id<ISSCComment> comment, NSError *error) {
...
{
    [Comment setUserInfoWithId:contentId
                      nickname:@"第三方用户"
